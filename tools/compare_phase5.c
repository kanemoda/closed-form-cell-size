...
static void run_scenario(scenario_id_t scen_id, int N, int frames) {
    ...
    run_adapter(&cfg, ADAPTER_MODE_BLIND, t, &res[n_methods].overhead_us,
                &res[n_methods].mean_ell, NULL);
    stats(t, frames, &res[n_methods]); res[n_methods].name = "Blind-9";
    n_methods++;

    fprintf(stderr, "  Closed-form Mode A ...\n"); fflush(stderr);
    run_adapter(&cfg, ADAPTER_MODE_A, t, &res[n_methods].overhead_us,
                &res[n_methods].mean_ell, NULL);
    stats(t, frames, &res[n_methods]); res[n_methods].name = "Closed-A";
    n_methods++;

    fprintf(stderr, "  Closed-form Mode B ...\n"); fflush(stderr);
    run_adapter(&cfg, ADAPTER_MODE_B, t, &res[n_methods].overhead_us,
                &res[n_methods].mean_ell, NULL);
